...
class ScreeningPDFGenerator:
    ...
    def _collect_files(self):
        ...
        if self.reports_directory.exists():
            ...
            for report_file in self.reports_directory.glob("*.pdf"):
                # Enhanced filtering to avoid re-adding ANY previously generated comprehensive reports
                filename_lower = report_file.name.lower()
                skip_patterns = [
                    "comprehensive_environmental_screening_report",  # Main reports
                    "enhanced_test_report",                          # Test reports  
                    "final_enhanced_report",                         # Final test reports
                    "comprehensive_screening_report"                 # Alternative naming
                ]
                
                # Skip if filename matches any comprehensive report pattern
                if any(pattern in filename_lower for pattern in skip_patterns):
                    print(f"  ⚠️ Skipping comprehensive report {report_file.name} (to avoid duplication)")
                    continue
                ...
                print(f"  📄 Added to inventory: {report_file.name} -> {category}")
